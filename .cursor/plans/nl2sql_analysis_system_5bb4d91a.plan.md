...
overview: 基于 FastAPI + LangChain 后端（通过 Qwen3 实现 NL2SQL）和 React 前端（三栏布局：会话管理、问答区域、右侧可视化图表+数据库工具）构建全栈智能数据分析系统。使用 conda 虚拟环境 nl2sql_vc（Python 3.11）。分为 5 个 Phase 递进开发。
todos:
...
- 右侧栏上下等分：上半区 ChartPanel（数据可视化），下半区 DatabasePanel（表结构浏览 + SQL 编辑器）
...
- SSE 事件流：`event: token` / `event: chart` / `event: done`
- 流程：加载记忆 -> Agent 执行 -> 流式输出 -> 图表生成 -> 保存记忆
